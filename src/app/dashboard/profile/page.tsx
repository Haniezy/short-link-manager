...
export default async function ProfilePage() {
  ...
  return <main className="dashboard-main profile-main"><Link href="/dashboard" className="detail-back">{fa ? "بازگشت به داشبورد" : "Back to dashboard"}<ArrowUpRight size={14} /></Link>
    <section className="dashboard-panel profile-hero"><div className="profile-identity"><span className="profile-avatar"><UserAvatar name={user.name || user.email} image={user.image} size={76} /><i /></span><div><span className="dashboard-eyebrow">{fa ? "حساب کاربری شما" : "YOUR ACCOUNT"}</span><h1>{user.name || (fa ? "پروفایل من" : "My profile")}</h1><p dir="ltr">{user.email}</p></div></div><div className="profile-stats"><div><Link2 size={17} /><strong>{result.data ? number.format(result.data.total) : "—"}</strong><span>{fa ? "لینک ساخته‌شده" : "Links created"}</span></div><div><MousePointer2 size={17} /><strong>{result.data ? number.format(result.data.totalClicks) : "—"}</strong><span>{fa ? "مجموع کلیک‌ها" : "Total clicks"}</span></div></div></section>
